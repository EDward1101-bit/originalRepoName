...
export default function DMsPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-primary)] text-[var(--text-normal)]" onClick={handlePageClick}>
      {/* Header */}
      <header className="h-12 flex items-center px-4 border-b border-[var(--border)] shrink-0 z-10 shadow-sm">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-[var(--text-muted)] text-[24px]">group</span>
          ...
          <div className="flex gap-4 items-center">
            ...
            >
              Pending {pendingReceived.length > 0 && <span className="bg-[var(--color-status-dnd)] text-white text-xs px-1.5 py-0.5 rounded-full ml-1">{pendingReceived.length}</span>}
            </button>
            <button 
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'Add Friend' && (
          <div className="p-8">
            ...
            <p className="text-[14px] text-[var(--text-muted)] mb-4">You can add friends with their username.</p>
            ...
            {searchQuery.trim() && (
              <div className="mt-8">
                <h3 className="text-xs font-bold text-[var(--text-muted)] uppercase mb-4">Search Results</h3>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            ...
                            <div className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-[2.5px] border-[var(--bg-primary)] ${u.online ? 'bg-[var(--color-status-online)]' : 'bg-[var(--color-status-dnd)]'}`} />
                          </div>
                        ...
                        <div>
                          {!u.relationship ? (
                            ...
                            u.relationship.receiver === myUsername ? (
                              ...
                              <button disabled className="px-4 py-1.5 bg-[var(--bg-modifier-active)] text-[var(--text-muted)] text-sm font-medium rounded">
                                Request Sent
                              </button>
                            )
                          ) : (
                            <button disabled className="px-4 py-1.5 bg-transparent border border-[var(--border)] text-[var(--text-muted)] text-sm font-medium rounded">
                              Added
                            </button>
                          )}
                        </div>
        ...
        {activeTab === 'Pending' && (
          <div className="p-8">
            <h2 className="text-xs font-bold text-[var(--text-muted)] uppercase mb-4">Pending — {pendingReceived.length}</h2>
            {pendingReceived.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-[var(--text-muted)]">
                <div className="w-32 h-32 bg-[var(--bg-tertiary)] rounded-full mb-6"></div>
                <p className="text-[16px]">There are no pending friend requests. Here's a Wumpus for now.</p>
              </div>
            ) : (
              <div className="flex flex-col">
                {pendingReceived.map((f) => (
                  <div key={f.id} className="flex items-center justify-between p-3 border-t border-[var(--border)] hover:bg-[var(--bg-modifier-hover)] rounded -mx-3 px-3 group transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[13px] text-[var(--text-muted)]">Incoming Friend Request</span>
                      </div>
                    </div>
        ...
        {activeTab === 'All' && (
          <div className="p-8">
            <h2 className="text-xs font-bold text-[var(--text-muted)] uppercase mb-4">All Friends — {acceptedFriends.length}</h2>
            {acceptedFriends.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-[var(--text-muted)]">
                <div className="w-32 h-32 bg-[var(--bg-tertiary)] rounded-full mb-6"></div>
                <p className="text-[16px]">Wumpus is waiting on friends. You don't have to though!</p>
              </div>
            ) : (
              <div className="flex flex-col">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-[16px]">{u.username}</span>
                          <span className="text-[13px] text-[var(--text-muted)]">{u.online ? 'Online' : 'Offline'}</span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button
